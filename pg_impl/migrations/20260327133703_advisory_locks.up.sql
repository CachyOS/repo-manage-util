-- Drop the redundant unique index; the table-level CONSTRAINT uniq_pkg already enforces this.
DROP INDEX IF EXISTS uniq_pkg_idx;

-- Replace upsert function with version that acquires an advisory lock first,
-- serializing concurrent upserts for the same (repo_name, pkg_name) pair.
...
-- Replace remove function with version that acquires an advisory lock first.
